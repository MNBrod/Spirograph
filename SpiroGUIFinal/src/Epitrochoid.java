...
public class Epitrochoid extends CenteredTrochoid {
  ...
  public Epitrochoid (int cx, int cy, int R, int r, int penSize,
    ...
    if (this.color == Color.ORANGE) {
      ...
      while (t < period) {
        int x = (int) ((fixedRingR + r) * Math.cos(t) - 
                       (period * Math.cos(((fixedRingR+r) *(1.0/r))*t)));
        int y = (int) ((fixedRingR + r) * Math.sin(t) - 
                       (period * Math.sin(((fixedRingR+r) *(1.0/r))*t)));
        g.setColor(Color.getHSBColor((.01f*q), 1f, 1f));
        g.drawOval(cx+x, cy+y, 2, 2);
        t = t + .001;
        q = q + (float)(1/period);
